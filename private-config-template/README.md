# heavy-truck-policy-daily-config

Private configuration for the public `heavy-truck-policy-daily` workflow.
...
4. Store that token as `PRIVATE_CONFIG_TOKEN` in the public repository; set `PRIVATE_CONFIG_REPO` to `qesbr/heavy-truck-policy-daily-config`.
