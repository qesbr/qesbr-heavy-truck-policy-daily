...
CARB、UNECE 等普通请求长期失败的公开页面由 GitHub Actions 临时启动
Playwright/Chromium 渲染。浏览器只读取无需登录的公开内容，不处理验证码、
不模拟账户登录，也不绕过访问控制。任务结束后浏览器及临时环境自动销毁。
浏览器来源单独失败时，其他官方 API、RSS、报告生成和网站发布仍会继续。
